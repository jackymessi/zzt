...
Ext.define('App.LoginPage', {
	...
	initComponent : function() {
		...
		this.items = [{
			height : 105,
			border : false,
			html : '<div style=\'background:url(extjs4/images/heat100.png) no-repeat left center; height:100%;'
		...
		Ext.Ajax.request({
					...
					success : function(response, options) {
                        ...
                        if (resp.success === false) {
                            ...
                            this.getEl().unmask();
                            Ext.util.Cookies.set('communityName', resp.community.name);
	...
	setUserCookies : function(form) {
		...
		// 记住checkbox
		Ext.util.Cookies.set('remember-loginName', reUsername ? 'Y' : 'N',
				expires);
	}
});